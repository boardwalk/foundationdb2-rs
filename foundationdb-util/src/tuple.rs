...
impl<'a, T1, T2> TuplePack for &'a (T1, T2)
where
    for<'x> &'x T1: TuplePack,
    for<'x> &'x T2: TuplePack,
{
    fn encode(self, out: &mut Vec<u8>, nested: bool) {
        let (ref v1, ref v2) = self;
...
where
    ...
    T2: TupleUnpack,
{
    fn decode(inp: &[u8], nested: bool) -> Result<((T1, T2), &[u8]), UnpackError> {
